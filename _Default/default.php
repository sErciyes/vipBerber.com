<?php
session_start();
require_once '../___class/DB.php'; // assuming you have a config.php file with your database connection details
require_once '../___class/RandevuManager.php';
class Berber {
...
function IsValidSaatFormat($saat) {
    // Saat formatı: HH:mm
    $saatRegex = '/^(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/';

    return preg_match($saatRegex, $saat);
}
if (isset($_POST['btnRandevuAl'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // POST verilerini al
        $secilenTarih = isset($_POST['Calendar1']) ? $_POST['Calendar1'] : '';
        $lsbSaat = isset($_POST['lsbSaat']) ? $_POST['lsbSaat'] : '';
        $txtAd = isset($_POST['txtAd']) ? $_POST['txtAd'] : '';
        $txtTelefon = isset($_POST['txtTelefon']) ? $_POST['txtTelefon'] : '';
        $txtAdres = isset($_POST['txtAdres']) ? $_POST['txtAdres'] : '';

        // Randevu al butonuna basıldığında işlemleri gerçekleştirelim
				...
				<div class="padding">
					...
					<form action="default.php" method="post">
						<div class="form-style-agile">
							...
							<button type="button" name="btnRandevuAl" class="btn btn-outline-success btn-lg btn-block">Success</button>
						</div>
					</form>
				</div>
			</div>
